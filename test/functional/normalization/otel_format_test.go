...
var _ = Describe("[Functional][Normalization] otel translation", func() {
	...
	BeforeEach(func() {
		...
		expectedMessage = normalizeJson(otelJson)
	})
	AfterEach(func() {
		framework.Cleanup()
	})
	It("should parse json message into structured", func() {
		ExpectOK(json.Unmarshal([]byte(otelJson), &expected))
		...
		// Log message data
		applicationLogLine := functional.CreateAppLogFromJson(otelJson)
	...
	It("It should check if the logs succesfully translated into otel", func() {
		framework = functional.NewCollectorFunctionalFrameworkUsingCollector(testfw.LogCollectionType)
		clfb = functional.NewClusterLogForwarderBuilder(framework.Forwarder).
			FromInput(logging.InputNameApplication).
